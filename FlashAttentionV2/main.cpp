...
static const torch::TensorOptions options[num_types] = {
    ...
    for (int t = 0; t < num_types; t++) {
        for (int i = 0; i < num_tests; i++) {
            printf("[%s] Params{.batch_size=%ld, .num_heads=%ld, .seq_len=%ld, .head_dim=%ld}:\n",
                (options[t].dtype() == torch::kFloat16 ? "FP16" : "BF16"),
                params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim
            );

            torch::Tensor q = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
            torch::Tensor k = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
            torch::Tensor v = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
            ...
            if (out_equal && lse_equal) {
                ...
                TIMING("TorchAttention", config<loop<5>>) {
                    std::ignore = torch::scaled_dot_product_attention(q, k, v);
                };
            }
        }
    }
...
void test_function() {
    ...
    for (int t = 0; t < num_types; t++) {
        for (int i = 0; i < num_tests; i++) {
            ...
            torch::Tensor q_base = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
            torch::Tensor k_base = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
            torch::Tensor v_base = torch::randn({params[i].batch_size, params[i].num_heads, params[i].seq_len, params[i].head_dim}, options[t]);
