...
int main() {
    ...
    {
        std::cout << "[FP16] Params{.batch_size=8, .num_heads=64, .seq_len=1000, .head_dim=128}:" << std::endl;
        Params params{.batch_size=8, .num_heads=64, .seq_len=1000, .head_dim=128};
        auto options = torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA);
    ...
    {
        std::cout << "[BF16] Params{.batch_size=8, .num_heads=64, .seq_len=1000, .head_dim=128}:" << std::endl;
        Params params{.batch_size=8, .num_heads=64, .seq_len=1000, .head_dim=128};
